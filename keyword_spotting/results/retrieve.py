...
def getCustomQuery(Labels, featureVectorList):
    queryList = list(open('../../query.txt', 'r'))
    querySize = 0
    queryLabelsOnce = ['' for i in range(15)]
    for i in range(len(queryList)):
        tokens = queryList[i].split('\n')
    ...
    for words in queryLabelsOnce:
        for i in range(len(Labels)):
            if (Labels[i] == words):
                queryLabels[counter] = words
                queryFeatures[counter] = featureVectorList[i]
                counter += 1
